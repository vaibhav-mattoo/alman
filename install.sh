...
main() {
    ...
    *)
    ...
    echo "Installed alman to ${BIN_DIR}"
...
usage() {
    ...
    echo "\
${_text_heading}alman installer${_text_reset}
Vaibhav Mattoo <vaibhavmattoo1@gmail.com>
https://github.com/vaibhav-mattoo/alman

Fetches and installs alman. If alman is already installed, it will be updated to the latest version.
...
download_alman() {
    ...
    local _releases_url="https://api.github.com/repos/vaibhav-mattoo/alman/releases/latest"
    local _releases
    case "${_dld}" in
    ...
    *) err "unsupported downloader: ${_dld}" ;;
    ...
    (echo "${_releases}" | grep -q 'API rate limit exceeded') &&
        err "you have exceeded GitHub's API rate limit. Please try again later, or use a different installation method: https://github.com/vaibhav-mattoo/alman/#installation"

    local _package_url
    _package_url="$(echo "${_releases}" | grep "browser_download_url" | cut -d '"' -f 4 | grep -- "${_arch}")" ||
        err "alman has not yet been packaged for your architecture (${_arch}), please file an issue: https://github.com/vaibhav-mattoo/alman/issues"
